...
const PERMISSION_LABELS: Record<string, { title: string; body: string }> = {
  ...
  'email:blob-read':  { title: 'Read raw message content',        body: 'Fetch the raw bytes of your messages and attachments (needed to decrypt and verify them). This also lets the plugin read the full contents of every file you attach to a message, before it is sent.' },
  'email:blob-write':  { title: 'Replace raw message content',        body: 'Replace the contents of a file you attached, after you attached it and before it is sent to the server (needed to encrypt attachments).' },
